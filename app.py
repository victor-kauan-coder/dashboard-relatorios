...
def criar_pdf_frequencia(df_monitor, nome_monitor, mes_ano, preceptora):
    ...
    # Adiciona a fonte 'DejaVu' para suportar caracteres UTF-8
    # NOTA: Você pode precisar ter um arquivo .ttf de uma fonte como 'DejaVu'
    # no seu repositório se 'Helvetica' falhar com nomes/acentos.
    # Por enquanto, usaremos Helvetica.
    ...
    pdf.cell(0, 5, f"MÊS DE REFERÊNCIA: {mes_ano.upper()}", ln=True) # [cite: 4]
    pdf.cell(0, 5, "Grupo Tutorial: Grupo 1 – Letramento para Usuários dos Serviços Digitais do SUS", ln=True) # [cite: 5]
    pdf.cell(0, 5, "Local de Atuação: CAPS AD – Teresina / PI", ln=True) # [cite: 5]
    pdf.cell(0, 5, f"Preceptora: {preceptora}", ln=True) # [cite: 5]
    ...
    for _, row in df_monitor.iterrows():
        ...
        saida = "18:00" # Fixo, baseado no template [cite: 6]
...
if not df.empty:
    ...
    if len(monitor_selecionado) == 1:
        ...
        if not df_filtrado.empty:
            ...
            preceptora = df_filtrado['Nome do preceptor'].iloc[0] # [cite: 5]
            
            # Pega o Mês/Ano da primeira entrada para o título
            mes_ano = df_filtrado['Data da atividade'].iloc[0].strftime('%B / %Y').capitalize() # [cite: 4]
